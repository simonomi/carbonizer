...
extension Bitmap: FileData {
	static var packedFileExtension = "bmp"
	static var unpackedFileExtension = "bmp"
	
	func toPacked() -> Datastream {
		let output = Datawriter()
		write(to: output)
		return output.intoDatastream()
	}
	
	func toUnpacked() throws -> Data {
		let output = Datawriter()
		write(to: output)
